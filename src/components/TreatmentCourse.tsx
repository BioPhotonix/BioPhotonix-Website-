    ...
    <div ref={ref} className="tech-card rounded-2xl p-6 md:p-8">
      <div className="flex flex-wrap items-end justify-between gap-6">
        <p className="leading-none">
          ...
          <span className="mt-2 block text-sm text-fog/60">minutes of treatment</span>
        </p>
        <p className="text-right leading-none">
          <span className="block text-3xl font-medium text-fog/85 md:text-4xl">
            {done}
            <span className="text-fog/45">/{total}</span>
          </span>
          <span className="mt-2 block text-sm text-fog/60">sessions delivered</span>
        </p>
      </div>
      ...
      <ol className="mt-8 flex flex-col gap-3">
        {Array.from({ length: course.weeks }, (_, w) => (
          <li key={w} className="flex items-center gap-4">
            <span className="w-16 shrink-0 text-xs font-semibold uppercase tracking-[0.13em] text-fog/50">
              Week {w + 1}
            </span>
                  ...
                  <span
                    ...
                    className={`relative flex h-11 flex-1 items-center justify-center rounded-lg border text-xs font-semibold transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${
                      filled
                        ...
                        : "border-line bg-ink-950 text-fog/25"
                    }`}
                  >
      ...
      <p className="mt-6 text-sm leading-relaxed text-fog/60">{course.footnote}</p>
    </div>
  );
}
